...
from temas.tema_amarelo_dnp import (
  COR_PRIMARIA, COR_TEXTO_TITULO, COR_FUNDO, COR_FUNDO_SECUNDARIA,
...
def build_relatorio(df: pd.DataFrame,
  ...
  imageGraficoProducaoDiaria        = graficoProducaoDiaria(stringDataInicio, stringDataFinal, df, "time", "volume_descarregado")
...
def main():
  ...
  WHERE cp.codigo_planta = :obra AND cp.`time` BETWEEN :ini AND :fim
  ORDER BY cp.`time`
  """
